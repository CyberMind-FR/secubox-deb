...
### 4. sbxwaf integration — the wake trigger (ARCHITECTURE PIVOT 2026-07-20)

**Decision:** the wake trigger lives in **sbxwaf (Go,
`secubox-toolbox-ng/cmd/sbxwaf`)**, NOT nginx. A `sleep` is the normal apply-stop
which **removes** the vhost's sbxwaf route (`haproxy-routes.json`); sbxwaf returns
**421** for a host with no route. The pivot: for a host in a known **on-demand
set**, sbxwaf reverse-proxies to the **waker** instead of 421. On wake, apply-start
re-adds the route (`portal_add`) and sbxwaf routes normally again. (The nginx
`@waker` snippet approach previously in this component is **superseded**;
`api/nginxgen.py` / `nginx-sync` stays in the tree but is not wired.)

**sbxwaf change** (`packages/secubox-toolbox-ng/cmd/sbxwaf`, built via
`packages/secubox-waf-ng`):
- New `OnDemand` config type (RWMutex + `map[string]bool`) loaded from
  `/etc/secubox/waf/on-demand-vhosts.json`, hot-reloaded via the same
  `reload.Watcher`/`Maybe()` pattern as `Routes`/`Rules`; new `--on-demand-vhosts`
  flag in `main()`.
- One branch at the existing 421 site in `handler()` (`main.go`, after
  `routeLookup` returns `ok=false`, before `http.Error(…421…)`): if
  `srv.onDemand.Contains(host)`, reverse-proxy to a lazily-built, cached
  unix-socket `*httputil.ReverseProxy` targeting `/run/secubox/waker.sock` at path
  `/_wake/<host>` (Transport `DialContext` → `net.Dial("unix", …)`, Director
  rewrites the path). Else fall through to the existing 421.
- The waker's 503 splash status is **excluded from the legitimate-visit tally**
  the same way 421/403 already are.
- AppArmor `/etc/apparmor.d/usr.sbin.sbxwaf` gains egress to
  `/run/secubox/waker.sock`. Deploy = rebuild `sbxwaf` + restart **both**
  `secubox-waf-ng-worker@1` and `@2` (never SIGHUP).

**On-demand-vhost-list generator** (`secubox-profiles`, replaces `nginx-sync`):
a `secubox-wakectl waf-sync` verb writes `/etc/secubox/waf/on-demand-vhosts.json`
= the set of `portal_domain`s whose module has `effective_lifecycle ∈
{eager, on-demand}`. Regenerated on any `lifecycle` change (postinst / webui).
