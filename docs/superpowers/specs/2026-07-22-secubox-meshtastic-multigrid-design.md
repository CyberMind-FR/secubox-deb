...
- **Deferred on-grid containment fixes** — the deferred egress-containment work must also (a) reconcile the default broker port: `gridpolicy.nft_egress_rules` renders the allow-rule for `:8883` when `on_grid.broker` carries no port, while `bridge.Bridge.start()` defaults to `:1883` in the same no-port case — pick one default and use it in both places; and (b) handle `nft ip daddr <hostname>`: nft resolves a DNS name only once at rule-load time, so a broker behind a multi-address/round-robin hostname needs an nft named set kept fresh (e.g. a refresh timer), not a bare `ip daddr <hostname>` literal.
