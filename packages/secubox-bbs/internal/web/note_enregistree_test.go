...
func TestLEnregistreurReutiliseLeCheminDuTrombone(t *testing.T) {
	...
	// Deux envois exactement : le trombone et l'enregistreur. Un troisieme
	// signalerait un chemin parallele.
	if n := strings.Count(js, "'/f/envoi'"); n != 2 {
		t.Errorf("%d appels a /f/envoi, attendu 2 (trombone + enregistreur)", n)
	}
