...
# Le poste de pilotage (#1322) porte publish/unpublish : ses routes doivent
# passer AVANT toute définition locale du même chemin.
...
def regenerate_nginx_config() -> tuple:
    ...
    for site in sites:
        ...
        # DÉPUBLIÉ = PAS DE BLOC (#1322). `published: false` dans site.json est
        # une intention explicite ; jusqu'ici le générateur l'ignorait et
        # « site unpublish » ne retirait jamais le bloc. Une clé ABSENTE reste
        # émise : les sites d'avant le schéma n'ont rien demandé.
        if sites_scan.read_site_config(Path(site["directory"])).get("published") is False:
            ecartes.append(f"{name} ({domain}) — dépublié (site.json)")
            continue
...
# Même dépôt pour le poste de pilotage (#1322), plus les chemins qu'il lit.
routers.pilotage.regenerer_nginx = regenerate_nginx_config
routers.pilotage.NGINX_METABLOGS_CONF = NGINX_METABLOGS_CONF
routers.pilotage.NGINX_ENABLED_DIR = NGINX_ENABLED_DIR
...
# publish / unpublish VIVENT DANS routers/pilotage.py (#1322). L'ancienne paire
# écrivait un `<nom>.conf` par site (sites-available + lien sites-enabled) : le
# modèle un-fichier-par-site, que le générateur unifié ne connaît que pour
# s'effacer devant lui (#1016) — deux blocs pour un domaine, et l'ordre
