...
SELECT p.id, p.source, p.titre, p.auteur, p.duree_ms,
       ...
       p.indisponible, p.raison,
       (SELECT COUNT(*) FROM coeurs c WHERE c.piste_id = p.id),
       COALESCE((SELECT MAX(l.debut_le) FROM lectures l WHERE l.piste_id = p.id), 0)
  FROM pistes p `
...
func (s *Store) scan(r interface{ Scan(...any) error }) (Piste, error) {
	...
	err := r.Scan(&p.ID, &p.Source, &p.Titre, &p.Auteur, &p.DureeMS,
		...
		&indispo, &p.Raison, &p.Coeurs, &p.JoueeLe)
...
// Toutes rend la playlist, ordre d'ajout.
func (s *Store) Toutes() ([]Piste, error) {
	rows, err := s.db.Query(selectPiste + `ORDER BY p.ajoute_le, p.id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
