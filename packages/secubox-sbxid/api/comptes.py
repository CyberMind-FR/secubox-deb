...
def _demande(pseudo: str, svc: str, action: str, password: str = "") -> Dict[str, Any]:
    d = {"service": svc, "action": action, "user": pseudo, "email": adresse(pseudo), "nom": pseudo}
    if password:
        d["password"] = password
    return d
...
def _pose_partout(c, uid, pseudo, pw, sauf: Optional[str] = None) -> Dict[str, Any]:
    ...
    for svc in liens(c, uid):
        if svc in SERVICES and svc != sauf:
            r = helper(_demande(pseudo, svc, "reinitialiser", pw))
...
def reinitialise(c: sqlite3.Connection, uid: str) -> Dict[str, Any]:
    ...
    if not res:
        raise Refus(409, "Aucun compte de service lié à cette personne")
