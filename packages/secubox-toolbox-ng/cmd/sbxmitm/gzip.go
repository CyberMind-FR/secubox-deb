...
func injectHTML(plain []byte, scriptBody, nonce string, wg bool, host string) []byte {
	// #740 : stripMetaTrustedTypes conserve (inutilise) pour reference — voir csp.go.
	_ = stripMetaTrustedTypes
	out := injectInlineBanner(plain, scriptBody, nonce)
	if wg {
		// #740 : cosmetique EasyList par-domaine — le host route la liste de filtres.
...
func injectIntoBody(body []byte, encoding, scriptBody, nonce string, wg bool, host string) (out []byte, ok bool) {
	switch strings.ToLower(strings.TrimSpace(encoding)) {
	case "":
		return injectHTML(body, scriptBody, nonce, wg, host), true
	case "gzip":
		plain, err := httpcodec.GunzipBytes(body)
		if err != nil {
			return body, false // fail open: serve the original compressed bytes
		}
		return httpcodec.GzipBytes(injectHTML(plain, scriptBody, nonce, wg, host)), true
	case "br":
		plain, err := httpcodec.UnbrotliBytes(body)
		if err != nil {
			return body, false // fail open
		}
		reenc, err := httpcodec.BrotliBytes(injectHTML(plain, scriptBody, nonce, wg, host))
		if err != nil {
	...
	case "zstd":
		...
		reenc, err := httpcodec.ZstdBytes(injectHTML(plain, scriptBody, nonce, wg, host))
		if err != nil {
