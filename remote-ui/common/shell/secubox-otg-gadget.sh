...
# Variant-aware. Set GADGET_NAME=secubox-<variant> (default "secubox") before
# invocation to use a distinct configfs gadget directory. round/ ships with the
# default; square/ (Phase 2) overrides via systemd unit Environment=.
readonly GADGET_NAME="${GADGET_NAME:-secubox}"
...
get_serial() {
    local s
    # Pi Zero W (BCM2835) — /proc/cpuinfo Serial line
    s=$(grep -oP 'Serial\s*:\s*\K[0-9a-f]+' /proc/cpuinfo 2>/dev/null || true)
    if [ -n "$s" ]; then
        echo "$s"
        return
    fi
