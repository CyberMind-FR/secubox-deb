...
# ══════════════════════════════════════════════════════════════════════════════
# PI ZERO METRICS API (pour dashboard Eye Remote)
# ══════════════════════════════════════════════════════════════════════════════

log "Installation Pi Zero Metrics API..."

# Copier l'API metrics
if [[ -f "$SCRIPT_DIR/pizero-metrics-api.py" ]]; then
    mkdir -p "$ROOT_MNT/opt/secubox"
    cp "$SCRIPT_DIR/pizero-metrics-api.py" "$ROOT_MNT/opt/secubox/"
    chmod +x "$ROOT_MNT/opt/secubox/pizero-metrics-api.py"
    log "pizero-metrics-api.py installé dans /opt/secubox/"
else
    warn "pizero-metrics-api.py non trouvé — création inline..."
    mkdir -p "$ROOT_MNT/opt/secubox"
    cat > "$ROOT_MNT/opt/secubox/pizero-metrics-api.py" << 'METRICSAPI'
#!/usr/bin/env python3
"""
...
# Copier le service systemd metrics
if [[ -f "$SCRIPT_DIR/pizero-metrics.service" ]]; then
    cp "$SCRIPT_DIR/pizero-metrics.service" "$ROOT_MNT/etc/systemd/system/"
    log "pizero-metrics.service installé"
else
    log "Création pizero-metrics.service inline..."
    cat > "$ROOT_MNT/etc/systemd/system/pizero-metrics.service" << 'METRICSSVC'
[Unit]
Description=SecuBox Pi Zero Metrics API
After=network.target

[Service]
Type=simple
ExecStart=/usr/bin/python3 /opt/secubox/pizero-metrics-api.py
Restart=always
RestartSec=5
User=root
...
# Activer le service metrics
mkdir -p "$ROOT_MNT/etc/systemd/system/multi-user.target.wants"
ln -sf /etc/systemd/system/pizero-metrics.service \
    "$ROOT_MNT/etc/systemd/system/multi-user.target.wants/pizero-metrics.service"

log "Pi Zero Metrics API configuré:"
log "  - API: http://10.55.0.2:8000/api/v1/status"
log "  - Health: http://10.55.0.2:8000/health"
