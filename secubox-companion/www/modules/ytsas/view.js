...
// SecuBox Companion :: ytsas — YouTube/web-media SAS (status + library + open).
//
// Read-only companion facet of the yt-dlp SAS: shows engine status and the
...
// (module.json + default mount()); the box proxies /api/v1/ytsas/* to the LXC.
function fmtBytes(n) {
  n = Number(n) || 0;
...
export default async function mount(ctx) {
  ...
  if (s) {
    grid.append(
      el('div.card', {}, [el('div.kicker', { text: 'en cours' }), el('h3', { text: String(s.downloading ?? s.active ?? '—') })]),
      el('div.card', {}, [el('div.kicker', { text: 'disque libre' }), el('h3', { text: fmtBytes(s.disk_free) })]),
      el('div.card', {}, [el('div.kicker', { text: 'cookies' }), el('h3', { text: s.cookies_present ? '✓ déposés' : '✕ absents' })]),
    );
  } else {
    grid.append(el('div.card', {}, [el('div.kicker', { text: 'statut' }),
      el('h3', { text: 'backend injoignable' }),
      el('div.muted', { style: 'font-size:.78rem', text: 'Le LXC ytsas ne répond pas sur cette box.' })]));
  }

  // Library list (title · progress · kept/ephemeral).
      ...
      for (const r of rows) {
        ...
        listWrap.append(el('div.card', { style: 'margin-bottom:8px' }, [
          el('div', { style: 'display:flex;justify-content:space-between;gap:10px' }, [
            el('strong', { style: 'font-size:.86rem;word-break:break-word', text: r.title || r.name || r.url || r.id || '—' }),
            el('span.badge', { text: tag }),
          ]),
          el('div.muted', { style: 'font-size:.76rem;margin-top:4px', text: `${pct}%${r.peertube_url ? ' · → peertube' : ''}` }),
        ]));
      }
    } else {
      listWrap.append(el('div.muted', { style: 'font-size:.82rem', text: 'Aucune vidéo dans le SAS.' }));
    }
  } catch (e) {
  ...
  root.append(el('div.row', { style: 'margin-top:14px' }, [
    el('a.btn', { href: 'https://ytsas.gk2.secubox.in', target: '_blank', rel: 'noopener', text: '🎞️ Ouvrir le SAS YouTube ↗' }),
  ]));
}
